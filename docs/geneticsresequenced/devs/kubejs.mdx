...
ServerEvents.registry('geneticsresequenced:gene', event => {
  event.createFromJson('coolmodpack:sun_powered', {
...
The generated translation keys are `gene.namespace.path` and `info.gene.namespace.path`. For the example above, those are `gene.coolmodpack.sun_powered` and `info.gene.coolmodpack.sun_powered`.
