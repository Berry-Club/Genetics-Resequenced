...
- `getTemporaryGeneTicks(entity, gene)` - Gets remaining temporary Gene ticks, or `0` if absent
- `isOnCooldown(entity, gene)` - Checks whether a Gene is on cooldown
- `getCooldownTicks(entity, gene)` - Gets remaining cooldown ticks, or `0` if absent
