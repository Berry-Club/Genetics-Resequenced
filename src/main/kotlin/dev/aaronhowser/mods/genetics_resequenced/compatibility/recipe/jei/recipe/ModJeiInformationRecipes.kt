package dev.aaronhowser.mods.genetics_resequenced.compatibility.recipe.jei.recipe
...
import dev.aaronhowser.mods.genetics_resequenced.datagen.lang.ModInfoLang
...
import dev.aaronhowser.mods.genetics_resequenced.gene.Gene.Companion.isMutation
	...
	private fun addMobGenes(registration: IRecipeRegistration) {
		...
		for (entityType in EntityDnaItem.VALID_ENTITY_TYPES) {
			...
			val stacks = buildList {
				...
				SpawnEggItem.byId(entityType).ifPresent { add(it.value().defaultInstance) }
			}
