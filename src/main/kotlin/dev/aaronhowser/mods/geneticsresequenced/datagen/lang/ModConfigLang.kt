...
object ModConfigLang {
	fun add(provider: ModLanguageProvider) {
		...
		provider.apply {
			...
			add(Client.DISABLE_CRINGE_LANG_CHANGE, "Disable cringe language")
			add(Client.SUPPORT_SLIME_RENDER_DEBUG, "Debug Support Slime rendering")
			add(Client.ITEM_MAGNET_BLACKLIST_TOOLTIP, "Item Magnet blacklist tooltip")

			add(Server.General.KEEP_GENES_ON_DEATH, "Keep Genes on death")
			add(Server.General.MINIMUM_COOLDOWN_FOR_NOTIFICATION, "Minimum cooldown for notification")
			add(Server.General.ANTIFIELD_BLOCK_RADIUS, "Anti-Field Block radius")

			add(Server.Machine.MACHINES, "Machines")
			add(Server.Machine.COAL_GENERATOR_ENERGY_PER_TICK, "Coal Generator Energy per tick")
			add(Server.Machine.COAL_GENERATOR_ENERGY_TRANSFER_RATE, "Coal Generator Energy transfer rate")
			add(Server.Machine.COAL_GENERATOR_ENERGY_CAPACITY, "Coal Generator Energy capacity")
			add(Server.Machine.INCUBATOR_TICKS_PER_BREW, "Incubator ticks per brew")
			add(Server.Machine.INCUBATOR_LOW_TEMP_TICK_FACTOR, "Incubator low temp tick factor")

			add(Server.Gene.GENES, "Genes")

			add(Server.Gene.DISABLED_GENES, "Disabled Genes")
			add(Server.Gene.DISABLE_GIVING_PLAYERS_NEGATIVE_GENES, "Disable giving players negative Genes")

			add(Server.Gene.BIOLUMINESCENCE_DURATION, "Bioluminescence duration")
			add(Server.Gene.BIOLUMINESCENCE_COOLDOWN, "Bioluminescence cooldown")
			add(Server.Gene.CHILL_DURATION, "Chill duration")
			add(Server.Gene.CHILL_CHANCE, "Chill chance")
			add(Server.Gene.MILKY_COOLDOWN, "Milky cooldown")
			add(Server.Gene.THORNS_DAMAGE, "Thorns damage")
			add(Server.Gene.THORNS_CHANCE, "Thorns chance")
			add(Server.Gene.THORNS_HUNGER_DRAIN, "Thorns hunger drain")
			add(Server.Gene.ITEM_MAGNET_COOLDOWN, "Item Magnet cooldown")
			add(Server.Gene.XP_MAGNET_COOLDOWN, "XP Magnet cooldown")
			add(Server.Gene.XP_MAGNET_RADIUS, "XP Magnet radius")
			add(Server.Gene.ITEM_MAGNET_RADIUS, "Item Magnet radius")
			add(Server.Gene.NO_HUNGER_COOLDOWN, "No Hunger cooldown")
			add(Server.Gene.PHOTOSYNTHESIS_COOLDOWN, "Photosynthesis cooldown")
			add(Server.Gene.PHOTOSYNTHESIS_SATURATION_AMOUNT, "Photosynthesis saturation amount")
			add(Server.Gene.PHOTOSYNTHESIS_HUNGER_AMOUNT, "Photosynthesis hunger amount")
			add(Server.Gene.NO_HUNGER_MINIMUM, "No Hunger minimum hunger")
			add(Server.Gene.WALL_CLIMB_SPEED, "Wall Climb speed")
			add(Server.Gene.PASSIVES_CHECK_COOLDOWN, "Passives check cooldown")
			add(Server.Gene.MOB_SIGHT_COOLDOWN, "Mob Sight cooldown")
			add(Server.Gene.MOB_SIGHT_RADIUS, "Mob Sight radius")
			add(Server.Gene.JOHNNY_ATTACK_MULTIPLIER, "Johnny attack multiplier")
			add(Server.Gene.SLIMY_DEATH_HEALTH_MULTIPLIER, "Slimy Death health multiplier")
			add(Server.Gene.TELEPORT_DISTANCE, "Teleport distance")
			add(Server.Gene.EGG_COOLDOWN, "Lay Egg cooldown")
			add(Server.Gene.EMERALD_HEART_COOLDOWN, "Emerald Heart cooldown")
			add(Server.Gene.SLIMY_DEATH_COOLDOWN, "Slimy Death cooldown")
			add(Server.Gene.SLIMY_DEATH_DESPAWN_TIME, "Slimy Death despawn time")
			add(Server.Gene.SLIMY_DEATH_DESPAWN_CHECK_TIMER, "Slimy Death despawn check timer")
			add(Server.Gene.CLAWS_DAMAGE, "Claws damage")
			add(Server.Gene.CLAWS_BASE_CHANCE, "Claws base chance")
			add(Server.Gene.TELEPORT_COOLDOWN, "Teleport cooldown")
			add(Server.Gene.MEATY_COOLDOWN, "Meaty cooldown")
