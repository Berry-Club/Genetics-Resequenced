...
object GenesModonomiconCategory {

	private lateinit var registries: HolderLookup.Provider

	lateinit var bookCategory: ModonomiconBookCategory
	lateinit var milky: ModonomiconBookEntry
	lateinit var itemAttractionField: ModonomiconBookEntry
	lateinit var noFallDamage: ModonomiconBookEntry
	lateinit var enderDragonHealth: ModonomiconBookEntry
	lateinit var scareCreepers: ModonomiconBookEntry
	lateinit var wooly: ModonomiconBookEntry
	lateinit var stepAssist: ModonomiconBookEntry
	lateinit var thorns: ModonomiconBookEntry
	lateinit var teleport: ModonomiconBookEntry
	lateinit var xpAttractionField: ModonomiconBookEntry
	lateinit var scareSkeletons: ModonomiconBookEntry
	...
	private fun addEntries(book: ModonomiconBook) {
		bookCategory.entry(
			...
			textPage(
				text = doubleSpacedLines(
					...
					"In the ${internalLink(BlocksModonomiconCategory.plasmidInfuser, "Plasmid Infuser")}, Basic Helices contribute 1 DNA Point. Mobs without a listed Gene pool always provide Basic."
				)
			)
		...
		enderDragonHealth = bookCategory.entry(
			...
			textPage(
				text = doubleSpacedLines(
					"The ${major("Ender Dragon Health")} Gene allows players to ${minor("completely negate damage")} while a ${internalLink(ItemsModonomiconCategory.dragonHealthCrystal, "Dragon Health Crystal")} is in their inventory.",
		...
		itemAttractionField = bookCategory.entry(
			...
			textPage(
				text = doubleSpacedLines(
					...
					"This Gene is disabled when the player has an active ${internalLink(ItemsModonomiconCategory.antiFieldOrb, "Anti-Field Orb")}, is near an active ${internalLink(BlocksModonomiconCategory.antiFieldBlock, "Anti-Field Block")}, or when sneaking.",
					"This Gene ${bad("cannot be given to mobs")}."
				)
			)
		...
		noFallDamage = bookCategory.entry(
		...
		scareCreepers = bookCategory.entry(
		...
		scareSkeletons = bookCategory.entry(
		...
		teleport = bookCategory.entry(
			...
			textPage(
				text = doubleSpacedLines(
					"The ${major("Teleport")} Gene allows players to ${minor("teleport forward")} when the Teleport key is used.",
		...
		xpAttractionField = bookCategory.entry(
