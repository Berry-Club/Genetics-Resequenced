...
object ModInfoLang {
	...
	fun add(provider: ModLanguageProvider) {
		provider.add(ORGANIC_MATTER, "Acquired by clicking a %1\$s with a Scraper.")
		provider.add(ORGANIC_MATTER_EMPTY, "Acquired by clicking an entity with a Scraper.\n\nFor more information, look at one that actually has an entity set 😉")
		provider.add(MOB_GENE_ONE, "%1\$s has these Genes:")
		provider.add(MOB_GENE_TWO, "\n%d%% of %2\$s")
		provider.add(REQUIRED_GENES, "Required Genes:")

		fun addGeneInfo(gene: ResourceKey<Gene>, info: String) {
			provider.add("info.${gene.translationKey}", info)
		}
