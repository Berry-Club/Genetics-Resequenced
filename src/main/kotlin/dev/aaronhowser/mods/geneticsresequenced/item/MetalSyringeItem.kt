    ...
    companion object {
            ...
            if (isContaminated(syringeStack)) {
                if (!pPlayer.level().isClientSide) {
                    pPlayer.sendSystemMessage(
                        Component.translatable(ModLanguageProvider.Messages.METAL_SYRINGE_CONTAMINATED)
                    )
                }
                return
        ...
        private fun tryInjectBlood(
            ...
            if (entityUuid != pInteractionTarget.uuid) {
                sendMessage(Component.translatable(ModLanguageProvider.Messages.METAL_SYRINGE_MISMATCH))
                return
            }
                ...
                for (gene in genesCantAdd) {
                    sendMessage(
                        Component.translatable(
                            ModLanguageProvider.Messages.METAL_SYRINGE_NO_MOBS,
                            gene.nameComponent
                        )
                    )
                }
